...
def normalize_item(value: str) -> str:
    ...
    value = re.sub(r"[0-9一二三四五六七八九十百千萬餘兩兩斤匹疋隻足枝副兠顆個人口艘錠]+", "", value)
    value = value.strip(" ：:。；;、，, /")
    return value
...
def split_tribute_items(value: str) -> list[str]:
    if not value:
        return []
    parts = re.split(r"[、，,；;。/]|並|及|等物|之屬|凡|又貢|合共計", value)
    items = []
    generic_terms = {
        "方物",
        "本國地圖",
...
"""
    ...
    write_js_file(
        OUT_DIR / "config.js",
        {
            "NANJING": {"name": "應天府", "lat": 32.06, "lng": 118.78},
            "MAP_CONFIG": {
            ...
            "TRIBUTE_CATEGORIES": {
                "animal": {"label": "動物", "icon": "elephant"},
                "spice": {"label": "香料", "icon": "incense"},
                "textile": {"label": "織物", "icon": "scroll"},
